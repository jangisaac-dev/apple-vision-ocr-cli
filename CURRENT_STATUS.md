...
- Agent-facing CLI contract (2026-09-13): every run path (single-process, split TXT, split PDF, split PDF+TXT) prints `Progress: N/M pages` on stderr per completed page (split paths wire `SplitProcessOCRRunner` `onProgress`; before this, split runs printed no progress at all). `main.swift` routes SIGINT/SIGTERM through `OCRJobControl`, so cancel stops workers, removes `$TMPDIR/apple-vision-ocr-split-*` (previously leaked on SIGTERM), leaves no partial output, and exits 130/143. `--help` lists the output contract and exit codes. Verified with real OCR on a 40-page fixture: split text byte-identical to the pre-change output, SIGTERM/SIGINT leave no processes/temp dirs/outputs, and the detached job recipe in `docs/ai-install-and-setup.md` works as written. Hardened after an adversarial review: `Progress:` counts are printed under a lock and only when they increase (pages finish on several threads, so updates arrived out of order), repeated signals are ignored instead of force-exiting past cleanup (SIGKILL still forces), and split children keep the default signal action so the parent's SIGTERM stops them at once (measured cancel latency 0.11s with 4 workers). Both gaps were fixed later on 2026-09-14: the split runner now counts progress only from child stderr lines that start with "Completed page " and never above the total page count, and after all split workers exit successfully, the runner waits at most 5 seconds for their output pipes to close instead of waiting forever.
- VOCR GUI localization (2026-09-13): the VOCR GUI (detail window, menu bar menu, alerts, log lines) is now English by default and Korean when the macOS preferred language starts with `ko` (users can change it per app in System Settings > General > Language & Region > Applications; there is no in-app switch). All GUI strings live in `Sources/VOCR/VOCRStrings.swift` (single table with English and Korean for every key), and `VOCR_LANGUAGE=en|ko` forces the GUI language as a verification seam (like `VOCR_HEADLESS`). The app bundle now declares `CFBundleDevelopmentRegion` `en` and `CFBundleLocalizations` `en`, `ko`. CI workflow was updated to `actions/checkout@v7` (Node.js 24), removing the Node.js 20 deprecation warning. The CLI output and the Finder Quick Action name (`Apple Vision OCR`) remain unchanged. Later on 2026-09-14, the Korean VOCR GUI no longer shows English for messages that were previously hard-coded or passed through from Core: the job-canceled message, the split-mode "OCR <file>" progress text, the eight Core pipeline progress messages (Starting OCR, Existing selectable text found; rasterizing affected pages, Writing output, Completed OCR, Rendering page N, Paused, OCR page N, Completed page N) which the GUI now maps to localized text when displaying them, the canceled-job error, and the "<file>: N page(s)" line in the existing-text alert, while Core and CLI output stay English (the CLI stderr contract is unchanged) and other Core error descriptions (which contain file paths and technical detail) still appear in English.
...
- 2026-09-13 (v1.2.0): `swift test` passed: 108 tests, 0 failures. `scripts/make-release-package.sh` produced `VOCR-1.2.0-macos-arm64.zip` (SHA-256 verified, no AppleDouble entries, no personal paths in either binary, `VOCR.icns` bundled, plist 1.2.0 / build 3, `codesign --verify --deep --strict` OK). Installed from the unzipped, quarantine-marked package with `env -i` and a PATH without Swift into a fake `HOME` with app dir `My Apps & Tools` and bin dir `bin dir`: no build ran, quarantine removed, installed codesign OK, app and CLI report 1.2.0, workflow plists lint OK, and the workflow bakes the shell-quoted, XML-escaped custom app path. Installed CLI: sample smoke text matches; 12-page fixture with `--split-workers 4` PDF+TXT exits 0 with 12 `Progress:` lines ending `12/12`. Packaged portable CLI: SIGTERM 3 s into a split run exits 143 with no workers, temp dirs, or partial output. After an external review, the documented quick-start block (subshell with `set -euo pipefail`, `curl -f`, `unzip -o`) was run against local copies of the release files: good checksum installs and a rerun in the same directory does not prompt; a corrupted zip stops at `shasum` (exit 1) and a missing file stops at `curl` (exit 37), with nothing installed. Two detached jobs from the `mktemp -d` recipe ran back to back with separate state (each `12/12`, exit 0). SIGTERM sent when `Writing output` appeared (single-process PDF, 40 pages, 3 tries) now exits 0 with the output and its stdout path, because `main.swift` keeps a successful result instead of reporting 143. Packaged `VOCR.app` with `VOCR_HEADLESS=1` on a 6-page fixture produced a 6-page `_ocr.pdf` in 6 s; `VOCR_HEADLESS=cancel` on 40 pages exited in 2 s with no output, processes, or temp dirs. Not verified: macOS 13, Intel, and the Gatekeeper dialog for a downloaded `Install.command`.
